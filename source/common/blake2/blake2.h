/* modified to remove blake2b, blake2xb, blake2bp, and blake2sp */
...
extern "C" {
  ...
  /*
     ...
     buffer, but those bytes end at buf[BLAKE2S_OUTBYTES]
     instead of starting at buf[0].
  */
  typedef struct blake2xs_finished_state__
  {
    blake2s_param P[1];
    uint8_t root[BLAKE2S_OUTBYTES];
    uint8_t buf[BLAKE2S_OUTBYTES];
    size_t  buflen;
  } blake2xs_finished_state;
